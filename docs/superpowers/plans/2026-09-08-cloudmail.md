...
Zero Trust → Access → Applications → Self-hosted, domaine `mail.planigramme.fr`, politique « Emails » limitée à `vous@example.com`. Copier l'Application Audience (AUD) dans `ACCESS_AUD` et le team domain dans `ACCESS_TEAM_DOMAIN`.
